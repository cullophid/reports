...
type Tool = "insert_chart" | "insert_image" | "insert_text" | "select";
...
export default () => {
  ...
  return (
            ...
            updateReport({
              ...
              slides: report.slides.map(slide => ({
                ...
                charts: slide.charts.filter(
                  chart =>
                    selection.some(selection => selection.id === chart.id) ===
                    false
                )
              }))
            });
    ...
    >
      {(reportQuery.loading || updateReportQuery.loading) && (
        <LoadingIndicator />
      )}
      <Header>
        <HeaderSection>
      ...
      <Stage tool={tool} scale={scale}>
        {selectedSlide && (
          ...
          >
            {["insert_chart", "insert_image", "insert_text"].includes(tool) &&
              isDragging && <DragBox {...getDragBox(dragPos, dragOrigin)} />}
              ...
              return (
                <Chart
                  ...
                  onClick={e => {
                    if (tool === "select") {
                      setSelection(
                        e.nativeEvent.shiftKey ? [...selection, chart] : [chart]
                      );
                    }
                  }}
                />
          ...
          <Center>
            <Button
              ...
              onClick={() => {
                updateReport({
                  ...report,
                  slides: [...report.slides, { id: cuid(), charts: [] }]
                });
              }}
            >
